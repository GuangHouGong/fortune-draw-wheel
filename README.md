...
- 支援匯入 `.xlsx`、`.csv`、`.txt` 名單；Excel 會讀第一個工作表，優先使用 `姓名`、`編號`、`name`、`id` 等欄位，否則讀第一個有資料的欄位。
...
3. 也可匯入 `.xlsx`、`.csv` 或 `.txt` 名單；`.xlsx` 請使用新版 Excel 格式，舊版 `.xls` 請先另存成 `.xlsx` 或 CSV。
4. 按「開始抽獎」啟動輪盤。
5. 再按「停止抽獎」開獎。
6. 中獎者會自動加入紀錄；預設不會再次參與下一輪。
7. 需要重複抽同一人時，開啟「允許重複中獎」。
8. 活動結束後可匯出 CSV。
